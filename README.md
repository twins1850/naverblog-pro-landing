...
- 라이선스 자동 생성 및 발급
- 웹사이트 이미지 최적화
- **뉴스레터 구독 시스템** (2024.11.10 추가)

**🚀 달성된 목표**: 결제부터 라이선스 발급까지 100% 무인 자동화 시스템 완성! 🎉

## 📰 뉴스레터 시스템 (2024.11.10 추가)

### ✨ 뉴스레터 기능
- 📧 **이메일 구독 관리**: 별도 Google Sheets에서 관리
- 🔒 **데이터 분리**: 고객 결제 데이터와 완전 분리
- 📊 **구독자 관리**: 활성/비활성 상태 관리
- 🎯 **관리자 페이지**: `/admin/newsletter`에서 구독자 관리

### 📁 뉴스레터 시스템 구조

```
lib/
├── newsletter-sheets.ts    # 뉴스레터 전용 Google Sheets 서비스

app/api/
├── newsletter/            # 뉴스레터 구독 API
└── admin/newsletter-subscribers/  # 관리자 API

components/ui/
└── newsletter-signup.tsx  # 뉴스레터 구독 폼 컴포넌트

app/admin/
└── newsletter/           # 뉴스레터 관리자 페이지
```

### 🔧 뉴스레터 환경 변수

```env
# 뉴스레터 전용 Google Sheets ID
NEWSLETTER_SPREADSHEET_ID=your-newsletter-sheet-id
```

### 📊 뉴스레터 시트 구조

| 구독일시 | 이메일 | 구독상태 | 구독ID | 수신동의 | 마케팅동의 | 구독경로 |
|---------|--------|----------|--------|----------|------------|----------|
| 2024-11-10 | user@example.com | 활성 | NL-xxx | Y | Y | 웹사이트 |

### 🔐 데이터 보안
- **완전 분리**: 고객 결제 데이터와 뉴스레터 데이터는 별도 시트에서 관리
- **독립 서비스**: `NewsletterSheetsService`로 독립적 운영
- **권한 관리**: 각 시트별 독립적인 접근 권한
