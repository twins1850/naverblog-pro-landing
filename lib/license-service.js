  ...
  async generateRealLicense(customerInfo) {
    try {
      ...
      const requestBody = {
        ...
        encoding_prefix: encodingPrefix,
        version: "v1",
      ...
      // 서버가 잘못된 인코딩을 반환했을 경우 수정
      if (data.license_key && data.license_key.startsWith('V3-') && encodingPrefix !== 'V3') {
        console.log(`🔧 라이선스 인코딩 수정: V3 → ${encodingPrefix}`);
        correctedLicenseKey = data.license_key.replace(/^V3-/, `${encodingPrefix}-`);
        console.log(`✅ 수정된 라이선스 키: ${correctedLicenseKey}`);
      } else if (!data.license_key.startsWith(encodingPrefix)) {
        // 다른 잘못된 인코딩도 수정
        const parts = data.license_key.split('-');
        if (parts.length >= 2) {
          parts[0] = encodingPrefix;
          correctedLicenseKey = parts.join('-');
          console.log(`🔧 라이선스 인코딩 수정: ${data.license_key.split('-')[0]} → ${encodingPrefix}`);
          console.log(`✅ 수정된 라이선스 키: ${correctedLicenseKey}`);
        }
      }
