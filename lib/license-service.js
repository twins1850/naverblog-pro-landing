...
export class LicenseService {
  ...
  async issueLicenseFromPayment(paymentInfo) {
    try {
      ...
      // PayAction 웹훅 기반 결제상태 업데이트
      try {
        await this.googleSheetsService.updateLicenseInfo(paymentInfo.orderId, {
          결제상태: "결제완료",
          라이센스키: licenseResult.licenseKey,
          발급일시: licenseResult.createdAt,
          만료일시: licenseResult.expireDate,
        ...
        console.log(`✅ PayAction 웹훅: 주문번호 ${paymentInfo.orderId} 결제상태를 "결제완료"로 업데이트 완료`);
      } catch (updateError) {
        console.error(`⚠️ PayAction 웹훅: 결제상태 업데이트 실패 (주문번호: ${paymentInfo.orderId}):`, updateError);
  ...
  async issueLicense(customerInfo) {
    try {
      ...
      // 라이선스 발급 후 결제상태를 "결제완료"로 업데이트
      try {
        await this.googleSheetsService.updateLicenseInfo(customerInfo.orderId, {
          결제상태: "결제완료",
          라이센스키: licenseKey,
          발급일시: new Date().toISOString(),
          만료일시: expiryDate,
        ...
        console.log(`✅ 주문번호 ${customerInfo.orderId} 결제상태를 "결제완료"로 업데이트 완료`);
      } catch (updateError) {
        console.error(`⚠️ 결제상태 업데이트 실패 (주문번호: ${customerInfo.orderId}):`, updateError);
        // 업데이트 실패해도 전체 프로세스는 계속 진행
      }
