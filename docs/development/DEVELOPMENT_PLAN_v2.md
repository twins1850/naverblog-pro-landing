...
### ✅ **4단계: 뉴스레터 구독 시스템 (완료 - 2025-01-09)**

- 이메일 구독 폼 컴포넌트 구현
- Google Sheets 자동 연동 (`newsletter-sheets.ts`)
- 뉴스레터 전용 시트 인증 시스템
- 실시간 구독자 데이터 저장
- 중복 이메일 방지 로직
- 구독 성공/실패 피드백 시스템
- 이메일 유효성 검증
- Google Analytics 이벤트 트래킹 연동

### ✅ **5단계: Google Analytics 통합 (완료 - 2025-01-10)**

- **완전 분리된 Analytics 계정 생성**: AutoToolsHub 전용 (뉴잉글리쉬와 완전 분리)
- **측정 ID 구현**: `G-QN3V4ZH4HL` (AutoToolsHub 전용)
- **환경 변수 설정**: `NEXT_PUBLIC_GA_MEASUREMENT_ID` Vercel 배포 완료
- **스크립트 최적화**: `beforeInteractive` 전략으로 빠른 로딩
- **이벤트 트래킹 시스템**: 포괄적인 이벤트 추적 라이브러리 (`lib/analytics.ts`)
  - 페이지뷰, 결제, 뉴스레터 구독, 라이선스 다운로드
  - 카카오톡 채널, 외부 링크, 폼 제출, 파일 다운로드
  - 검색, 비디오 재생, 스크롤 깊이, 성능 측정
  - Enhanced Ecommerce 완전 지원
- **실시간 데이터 수집**: 정상 동작 확인 완료
- **데이터 분리 보장**: 뉴잉글리쉬(new1eng.com) 데이터와 100% 분리
